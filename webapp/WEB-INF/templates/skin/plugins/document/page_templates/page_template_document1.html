			...
			<div id="toolbar" class="row hidden-print">
				...
				<div id="project-elaboration-src" class="hidden">
					...
					<#if listChilds?size &gt; 0> <#-- Projet issu d'une proposition co-construite -->
						<div id="project-elaboration">
							...
							<#list listChilds as child>
								<p>
									<a href="jsp/site/Portal.jsp?page=proposal&campaign=${child.codeCampaign!''}&proposal=${child.codeProposal!''}" >
										${child.titre!''}
									</a>
								</p>
							</#list>
					...
					<#elseif idea?? >  <#-- Projet issu d'une proposition non co-construite -->
						...
						<p>
							<a href="jsp/site/Portal.jsp?page=proposal&campaign=${idea.codeCampaign!''}&proposal=${idea.codeProposal!''}" >
								${idea.titre!''}
							</a>
						</p>
					</#if>
				...
				<div id="vote-src" class="hidden">
					...
					<#-- Displaying button only if current campaign -->				
					<#assign currentCampaignCode = 2014 + "ABCDEFGHIJKLMNOPQRSTUVWXYZ"?index_of(campaignService.lastCampaign.code)>
					<#if campaign_text_document == ("Budget Participatif " + currentCampaignCode)> 
					
						<div class="psoum-data-vote">

							<#if campaignService.isDuring("VOTE") = true && voteValidated= true >
							...
							<#elseif campaignService.isDuring("VOTE") && voteValidated= false && (arrondissementVote == location_text || location_text= "Tout Paris")>
								@Extender[${id_document},document,vote,{show:"all"}]@
				...
				<input type="hidden" name="location_${id_document}" id="location_${id_document}" value="${location_text!}">
				<input type="hidden" name="title_${id_document}" id="title_${id_document}" value="${title_text!}">
				<input type="hidden" name="theme_${id_document}" id="theme_${id_document}" value="${theme_text!}">
				...
				<input type="hidden" name="campaign_text_document_${id_document}" id="campaign_text_document_${id_document}" value="${campaign_text_document!}">
