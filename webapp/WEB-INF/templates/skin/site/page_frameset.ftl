	...
	<nav id="main-navbar-bp" class="navbar-fixed-top" role="navigation">
		...
		<div class="collapse navbar-collapse" id="navbar-collapse-main">
			<ul>
				...
				<#if campaignService.isDuring("IDEATION")>
					<li>
				...
				<#if campaignService.isAfterBeginning("IDEATION") >
					<li>
						<a href="jsp/site/Portal.jsp?page=solrProjectSearch&amp;sort_name=${RANDOM!123456789}_random&amp;conf=list_proposals&amp;fq=campaign_text:${campaignService.lastCampaign.code}">
							<span>#i18n{portal.openpb.navbar.entry.getSubmittedProposals}</span>
						</a>
					</li>
				</#if>
				
				<#if campaignService.isAfterBeginning("SUBMIT") && campaignService.isBeforeBeginning("RESULT") >
					<li>
						<a href="jsp/site/Portal.jsp?page=solrProjectSearch&amp;sort_name=${RANDOM!123456789}_random&amp;conf=projects_mdp">
							<span><#if campaignService.isDuring("VOTE")>#i18n{portal.openpb.navbar.entry.doVote}<#else>#i18n{portal.openpb.navbar.entry.getProjectsUnderVote}</#if></span>
						</a>
					</li>
				</#if>
				
				<#if campaignService.isDuring("RESULT") >
					<li>
...
<script>
	$( function(){
		...
		/* QP PopOver*/
		if ($(".img-qp").length > 0){
			$(".img-qp").popover({placement: "right", container:"body", trigger:"click",html:"true", template:'<div class="popover qp" role="tooltip"><div class="arrow"></div><div class="popover-content"></div></div>', content:"#dskey{ideation.site_property.view_proposal.userInfoPopQP.htmlblock}"});
		}

		/* RATP Popover*/
		if ($(".img-ratp").length > 0){
			$(".img-ratp").popover({placement: "right", container:"body", trigger:"click",html:"true", template:'<div class="popover ratp" role="tooltip"><div class="arrow"></div><div class="popover-content"></div></div>', content:"#dskey{ideation.site_property.view_proposal.userInfoPopRATP.htmlblock}"});
		}
		...
		*/
		...
		else
		{
			var arr = {
				...
				"page=proposal"                 : "conf=list_proposals",
